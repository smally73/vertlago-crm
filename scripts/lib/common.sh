...
# Échoue si la branche donnée a des changements non commités ou non poussés/tirés.
require_clean_branch() {
  local branch="$1"
  if [[ -n "$(git status --porcelain)" ]]; then
    log_error "Working tree non propre. Commite ou stash tes changements avant de promouvoir."
    exit 1
  fi
  local current
